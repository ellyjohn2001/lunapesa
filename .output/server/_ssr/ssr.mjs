...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-CgkO86vm.mjs").then((m) => m.default ?? m);
...
function isKeepAliveRequest(request) {
	return new URL(request.url).pathname === "/health";
}
var server_default = { async fetch(request, env, ctx) {
	if (isKeepAliveRequest(request)) return new Response("ok", { status: 200 });
	try {
